...
class StatusBadge(tk.Canvas):
    def __init__(self, parent, status, width=90, height=24, **kwargs):
        ...
        bg_hex = self._blend_with_surface(self.color)
        self.create_rounded_rect(0, 0, width, height, height // 2,
                                 fill=bg_hex, outline='')
        self.create_text(width // 2, height // 2, text=self.text,
                         fill=self.color, font=FONTS['badge'])
...
class StatCard(tk.Frame):
    def __init__(self, parent, icon, label, value, color, command=None, **kwargs):
        super().__init__(parent, bg=COLORS['card'], padx=16, pady=14,
        ...
        icon_canvas = tk.Canvas(top, width=36, height=36,
                                bg=COLORS['card'], highlightthickness=0)
        icon_canvas.pack(side='left')
        icon_canvas.create_oval(2, 2, 34, 34, fill=color, outline='')
        icon_canvas.create_text(18, 18, text=icon, fill=COLORS['text_inverse'],
                                font=('Segoe UI', 12, 'bold'))

        info = tk.Frame(top, bg=COLORS['card'])
        info.pack(side='left', padx=(10, 0), fill='x', expand=True)

        tk.Label(info, text=label, bg=COLORS['card'], fg=COLORS['text_muted'],
                 font=FONTS['small'], anchor='w').pack(anchor='w')
        tk.Label(info, text=str(value), bg=COLORS['card'], fg=COLORS['text'],
                 font=('Segoe UI', 22, 'bold'), anchor='w').pack(anchor='w', pady=(2, 0))

        if command:
            self.bind('<Button-1>', lambda event: command())
            self.bind('<Enter>', self._on_enter)
            self.bind('<Leave>', self._on_leave)
            self._bind_children(self, command)

    def _bind_children(self, widget, command):
        for child in widget.winfo_children():
            child.bind('<Button-1>', lambda event: command())
            if child.winfo_children():
                self._bind_children(child, command)

    def _on_enter(self, event=None):
        self._set_background(COLORS['card_hover'])

    def _on_leave(self, event=None):
        self._set_background(COLORS['card'])
...
class ItemRow(tk.Frame):
    def __init__(self, parent, item, on_click=None, controller=None, **kwargs):
        super().__init__(parent, bg=COLORS['card'], padx=14, pady=12,
        ...
        self.img_label = tk.Label(self, bg=COLORS['card'], text='📷',
                                  width=4, font=('Segoe UI', 16))
        self.img_label.pack(side='left', padx=(0, 12))
        ...
        if item.get('photo_path'):
            try:
                ...
                self.img_label.config(image=photo, text='', width=46, height=46)
            except Exception as exc:
                self.img_label.config(text='⚠', fg=COLORS['danger'])
        ...
        tk.Label(info, text=location, bg=COLORS['card'],
                 fg=COLORS['text_muted'], font=FONTS['small']).pack(anchor='w', pady=(3, 0))

        if item.get('person'):
            tk.Label(info, text=f"With {item['person']}", bg=COLORS['card'],
        ...
        badge = StatusBadge(right, status_text, width=82)
        badge.pack(anchor='e')

        date_text = item.get('date_added', '').split()[0] if item.get('date_added') else ''
        tk.Label(right, text=date_text, bg=COLORS['card'],
                 fg=COLORS['text_muted'], font=FONTS['small']).pack(anchor='e', pady=(5, 0))

        if self.on_click:
            self._bind_all_clicks(self, self.on_click)
        ...
        if self.controller:
            ...
            self.img_label.bind("<Button-3>", self.context_menu.show)

    def _bind_all_clicks(self, widget, command):
        widget.bind('<Button-1>', lambda event: command(self.item))
        for child in widget.winfo_children():
            self._bind_all_clicks(child, command)
