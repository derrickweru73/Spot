...
class BorrowedView(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(header, text="Borrowed", bg=COLORS['bg'],
                 font=FONTS['title'], fg=COLORS['text']).pack(side='left')
        RoundedButton(header, text="+ Borrowed", command=self._add_borrowed,
                      bg=COLORS['info'], width=110).pack(side='right')
    ...
    def _add_borrowed(self):
        self.controller.open_add(default_status='borrowed')
    ...
    def _load_items(self):
        ...
        items = get_all_items(status='borrowed', search=self.search_text)

        if not items:
            tk.Label(self.list_frame, text="Nothing borrowed. Debt free!",
