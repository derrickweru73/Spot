...
class StashView(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(header, text="My Items", bg=COLORS['bg'],
                 font=FONTS['title'], fg=COLORS['text']).pack(side='left')
        RoundedButton(header, text="+ Add", command=self.controller.open_add,
                      bg=COLORS['primary'], width=90).pack(side='right')
    ...
    def _load_items(self):
        ...
        items = get_all_items(status='stored', search=self.search_text)

        if not items:
            tk.Label(self.list_frame, text="No items found.", bg=COLORS['card'],
